...
int copy_image(int dir, unsigned long image_size) 
{
	...
	if (dir == 1) {
		...
		len = 0x400000 - CFG_FIRMWARE_TOTAL_RESERVED_SIZE;
		if (image_size <= len) {
...
/* enter web failsafe mode, added by hubo, July 1st 2014 */
	...
	if(BootType == '3') {
		...
		case '2':
			...
			//erase linux
			if (NetBootFileXferSize <= (0x400000 - CFG_FIRMWARE_TOTAL_RESERVED_SIZE)) {
				...
				e_end = PHYS_FLASH_2 + NetBootFileXferSize - (0x400000 - CFG_FIRMWARE_TOTAL_RESERVED_SIZE);
				if (0 != get_addr_boundary(&e_end))
					break;
				printf("Erase linux kernel block !!\n");
...
#else
			if (NetBootFileXferSize <= (bd->bi_flashsize - CFG_FIRMWARE_TOTAL_RESERVED_SIZE)) {
				e_end = CFG_KERN_ADDR + NetBootFileXferSize;
				if (0 != get_addr_boundary(&e_end))
					break;
				printf("Erase linux kernel block !!\n");
